...
class Record:
    ...
    def __str__(self):
        print("%s.%s" % (self.SubDomain, self.Domain))

    def GetRecordIP(self):
        recordDomian = "%s.%s" % (self.SubDomain, self.Domain)
        ...
        try:
            ...
            if curIP:
                if curIP != localIP:
                    ...
                    if statusCode == "1":
                        self.PrintLog("DDNS updated to new ip: %s" % localIP)
                        curIP = localIP
                        ...
                        self.NextRunTime = datetime.datetime.now() + datetime.timedelta(seconds=60)
                        raise Exception("DNSPod return error status code: %s" % statusCode)
                else:
                    self.PrintLog("DDNS already up-to-date: %s" % localIP)
                    self.NextRunTime = datetime.datetime.now() + datetime.timedelta(seconds=60)
...
def LoadConfig(fileName):
    ...
    if fileList:
        print("Config file loaded: %s" % fileList)
        records = []
        for section in config.sections():
            ...
            record.PrintLog("Config loaded : %s.%s" % (record.SubDomain, record.Domain))
        return records
    else:
